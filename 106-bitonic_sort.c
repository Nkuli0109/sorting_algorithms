...
/**
 * merge - sort bionic sequences recursively in both orders
 * @arr: array
 * @low: first elemt
 * @nelemnt: elements number
 * @order: 1: ascending order, 0 descending order
 */
void merge(int arr[], int low, int nelemnt, int oredr)
{
	int mid, i;
...
/**
 * bitonicsort - bitonic sirt algorithm implementation
 * @arr: array
 * @low: first element
...
void bitonicsort(int arr[], int low, int nelemnt, int order, int size)
{
	...
	if (nelemnt > 1)
	{
		...
		bitonicsirt(arr, low = mid, mid, 0, size);
